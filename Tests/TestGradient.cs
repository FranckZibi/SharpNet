...
namespace SharpNetTests
{
    ...
    public class TestGradient
    {
        ...
        private static double ComputeGradientAndReturnLoss(Network network, CpuTensor<float> X, CpuTensor<float> yExpected, bool isTraining)
        {
            var yPredicted = (CpuTensor<float>)network.Predict(X, isTraining);
            network.BackwardPropagation(yExpected); //we compute gradients
            double result = 0;
