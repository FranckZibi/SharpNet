...
namespace SharpNetTests.NonReg
{
    ...
    public class TestNetworkPropagation
    {
        ...
        public static CpuTensor<float> FromNumpyArray_and_Transpose(string s)
        {
            var a  = (CpuTensor<float>)TensorExtensions.FromNumpyArray(s);
            var a_transpose = a.Clone();
            a.Transpose(a_transpose);
            return (CpuTensor<float>)a_transpose;
        }
