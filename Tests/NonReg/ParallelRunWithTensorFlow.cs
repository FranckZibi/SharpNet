...
namespace SharpNetTests.NonReg
{
    ...
    public class ParallelRunWithTensorFlow
    {
        ...
        public void TestParallelRunWithTensorFlow_UnivariateTimeSeries()
        {
            ...
            //series = new CpuTensor<float>(series.Shape, series.ContentAsFloatArray().Select(t => t / 100).ToArray());
            using var dataSet = new UnivariateTimeSeriesDataSet(series.Content, timeSteps, 1);
            //var trainingAndValidation = dataSet.SplitIntoTrainingAndValidation(1000 / 1400.0);


            //double learningRate = 0.00025;
            double learningRate = 0.0014;
            const double lambdaL2Regularization = 0.00;
            const double momentum = 0.9;
            int inputSize = 1;
            var deviceId = 0;
            ...
            const bool timeMajor = true;
            network
                .Input(timeSteps, inputSize, -1)
                .Linear(aNormalization, bNormalization)
                .LSTM(32, true, true, timeMajor)
                .LSTM(32, false, true, timeMajor)
                .Dense(1, 0.0, true)
            ...
            TestNetwork.Fit(network, dataSet, learningRate, numEpochs, batchSize, null);
            
            Log.Info("C# Default MAE = " + dataSet.DefaultMae);
            Log.Info("C# numEpochs= " + numEpochs);
            Log.Info("C# learningRate= " + learningRate);
            Log.Info("C# l2regularizer= " + lambdaL2Regularization);
            Log.Info("C# momentum= " + momentum);
