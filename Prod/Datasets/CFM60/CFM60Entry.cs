...
namespace SharpNet.Datasets.CFM60
{
    ...
    public class CFM60Entry
    {
        ...
        public static CFM60Entry[] Load(string xFile, string yFileIfAny, Action<string> log, Cfm60NetworkSample.ValueToPredictEnum valueToPredict, string[] predictionFilesIfComputeErrors)
        {
            ...
            if (File.Exists(protoBufFile))
            {
                ...
                if (valueToPredict == Cfm60NetworkSample.ValueToPredictEnum.ERROR)
                {
                    ReplaceYByError(result, log, predictionFilesIfComputeErrors);
                }
                else if (valueToPredict == Cfm60NetworkSample.ValueToPredictEnum.Y_TRUE_MINUS_LR)
                {
                    log("Trying to predict error compared to linear regression estimate");
                    ReplaceYByLinearRegressionEstimate(result);
                }
                else if (valueToPredict == Cfm60NetworkSample.ValueToPredictEnum.Y_TRUE_MINUS_ADJUSTED_LR)
                {
