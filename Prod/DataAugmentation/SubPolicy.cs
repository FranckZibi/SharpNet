    ...
    public static class SubPolicy
    {
            ...
            ImageDataGenerator.FillModeEnum fillMode)
        {
            ...
            for (int i = 0; i < subPolicy.Count; ++i)
            {
                ...
                var unconvertCX = policy.Unconvert_Slow(0, 0).row;
                var unconvertAX = policy.Unconvert_Slow(1, 0).row - unconvertCX;
                var unconvertBX = policy.Unconvert_Slow(0, 1).row - unconvertCX;
                unconvertCX += 1e-8;

                var unconvertCY = policy.Unconvert_Slow(0, 0).col;
                var unconvertAY = policy.Unconvert_Slow(1, 0).col - unconvertCY;
                var unconvertBY = policy.Unconvert_Slow(0, 1).col - unconvertCY;
                unconvertCY += 1e-8;
